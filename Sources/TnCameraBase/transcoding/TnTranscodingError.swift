...
public enum TnTranscodingError: Error {
    ...
    public static func tryStatus(status: OSStatus) throws {
        if let error = TnTranscodingError(status: status) {
            throw error
        }
    }
    
    public static func tryStatus(action: @escaping () -> OSStatus) throws {
        let status = action()
        if let error = TnTranscodingError(status: status) {
            throw error
        }
